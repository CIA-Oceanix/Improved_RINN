...
def generate_data(GD):
    ...
    if (GD.model == 'Lorenz_63'):
        ...
        # 5 time steps (to be in the attractor space)       
        x0 = np.array([4,1.1,4]);
        S = odeint(oregonator,x0,np.arange(0,10000+0.000001,GD.dt_integration),args=(GD.parameters.q,GD.parameters.f,GD.parameters.eps,GD.parameters.eps2));
        x0 = [1,0,0]

        # generate true state (xt)
        S = odeint(oregonator,x0,np.arange(0.01,GD.nb_loop_test+0.000001,GD.dt_integration),args=(GD.parameters.q,GD.parameters.f,GD.parameters.eps,GD.parameters.eps2));
        T_test = S.shape[0];      
        t_xt = np.arange(0,T_test,GD.dt_states);       
        xt.time = t_xt*GD.dt_integration;
        xt.values = S[t_xt,:];
        ...
        #generate catalog
        S =  odeint(oregonator,S[S.shape[0]-1,:],np.arange(0.01,GD.nb_loop_train+0.000001,GD.dt_integration),args=(GD.parameters.q,GD.parameters.f,GD.parameters.eps,GD.parameters.eps2));
        T_train = S.shape[0];
        eta = np.random.multivariate_normal(np.zeros(3),GD.sigma2_catalog*np.eye(3,3),T_train);
        catalog_tmp = S+eta;
        catalog.analogs = catalog_tmp[0:-GD.dt_states,:];
